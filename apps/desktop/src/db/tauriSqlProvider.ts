...
async clearWatchHistory(): Promise<void> {
  ...
  async markChangesSynced(ids: number[]): Promise<void> {
    ...
    await this.db!.execute(`UPDATE change_log SET synced = 1 WHERE id IN (${ph})`, ids);
  }

  async getMaxChangeLogId(): Promise<number> {
    const row = await this.selectOne<{ id: number | null }>('SELECT MAX(id) as id FROM change_log');
    return row?.id ?? 0;
  }

  async getLastChangeTime(tableName: string, recordId: string): Promise<number | null> {
    const row = await this.selectOne<{ ts: number | null }>(
  ...
  /** 把 recordId 拆成各表真实列（复合键/业务键），供读/删复用。 */
  ...
  async readSyncRecord(tableName: string, recordId: string): Promise<any | null> {
    ...
    if (!cond) return null;
    const row = await this.selectOne<any>(`SELECT * FROM ${tableName} WHERE ${cond.where}`, cond.params);
    if (!row) return null;
  ...
  async readMediaSubtree(mediaId: string): Promise<{ episodes: any[]; playSources: any[] }> {
    const episodes = await this.db!.select<any[]>('SELECT * FROM episode WHERE media_id = ?', [mediaId]);
    const playSources = await this.db!.select<any[]>(
  ...
  async rebuildMediaSubtree(mediaId: string, sourceIds: string[], subtree: MediaSubtreeSnapshot): Promise<void> {
    ...
    try {
      for (const sid of sourceIds) {
        await db.execute('DELETE FROM episode WHERE media_id = ? AND source_id = ?', [mediaId, sid]);
      }
      ...
      for (const ep of subtree.episodes ?? []) {
        ...
        await db.execute(buildUpsertSql('episode', cols, 'id'), cols.map((c) => row[c]));
      }
      for (const ps of subtree.playSources ?? []) {
        ...
        await db.execute(buildUpsertSql('play_source', psCols, 'id'), psCols.map((c) => row[c]));
      }
  ...
  async applyRemoteChange(tableName: string, recordId: string, operation: ChangeLog['operation'], data: any | null): Promise<boolean> {
    ...
    if (tableName === 'media' && data.subtree && Array.isArray(data.subtree.episodes)) {
      const existing = await this.selectOne<{ id: string }>(
  ...
  async upsertSyncRecord(tableName: string, row: any): Promise<void> {
    ...
    if (rule.dedupeBeforeInsert) {
      await this.db!.execute(`DELETE FROM ${tableName} WHERE ${rule.recordKey} = ?`, [row[rule.recordKey]]);
      await this.db!.execute(buildInsertSql(tableName, cols), values);
    } else {
      await this.db!.execute(buildUpsertSql(tableName, cols, rule.conflictTarget), values);
    }
  }
  ...
  async deleteSyncRecord(tableName: string, recordId: string): Promise<void> {
    const split = this.splitRecordId(tableName, recordId);
    if (split) {
      await this.db!.execute(`DELETE FROM ${tableName} WHERE ${split.where}`, split.params);
      return;
    }
    if (tableName === 'media') {
      await this.db!.execute('DELETE FROM media WHERE fingerprint = ? OR (fingerprint IS NULL AND id = ?)', [recordId, recordId]);
      return;
    }
    if (tableName === 'video_source') {
      await this.db!.execute('DELETE FROM video_source WHERE code = ? OR (code IS NULL AND id = ?)', [recordId, recordId]);
      return;
    }
    const rule = SYNC_UPSERT_RULES[tableName];
    if (rule) await this.db!.execute(`DELETE FROM ${tableName} WHERE ${rule.recordKey} = ?`, [recordId]);
  }

  async clearEchoChanges(sinceId: number): Promise<number> {
    const r = await this.db!.execute('DELETE FROM change_log WHERE synced = 0 AND id > ?', [sinceId]);
  ...
  async getSystemConfigValue(key: string): Promise<string | null> {
    const row = await this.selectOne<{ value: string }>('SELECT value FROM system_config WHERE key = ?', [key]);
  ...
  async deleteUnsyncedByTables(tables: string[]): Promise<number> {
    ...
    const r = await this.db!.execute(`DELETE FROM change_log WHERE synced = 0 AND table_name IN (${ph})`, tables);
    return r.rowsAffected ?? 0;
  }
